...
const babylon_lightscript = require("@oigroup/babylon-lightscript");
...
for (const jsFile of jsFiles) {
  ...
  traverse(ast, {
    enter(path) {
      ...
      if (node) {
        ...
        if ( !node.loc ) {
          problem("sourcemap", "node.loc is missing.");
        } else if (node.start == null) {
          problem("sourcemap", "node.start is missing");
        } else if (node.end == null) {
          problem("sourcemap", "node.end is missing");
        } else {
          if ( (otherNode = path.getPrevSibling().node) ) {
            if (otherNode.start && node.start < otherNode.start) {
              problem("sourcemap", "Starts before prevSibling starts.");
            }

            if (otherNode.end && node.start < otherNode.end) {
              problem("sourcemap", "Starts before prevSibling ends.");
            }
          }
          ...
          if ( path.parentPath && (otherNode = path.parentPath.node) ) {
            if (otherNode.start && node.start < otherNode.start) {
              ...
              if (otherNode.type !== "MemberExpression") {
                problem("sourcemap", "Starts before parent starts.");
              }
            }

            if (otherNode.start && node.end < otherNode.start) {
              problem("sourcemap", "Ends before parent starts.");
            }

            if (otherNode.end && node.start > otherNode.end) {
              problem("sourcemap", "Starts after parent ends.");
            }
            ...
            if (otherNode.end && node.end > otherNode.end) {
              ...
              if (otherNode.type !== "ReturnStatement") {
                problem("sourcemap", "Ends after parent ends.");
              }
            }
          }
        }
  ...
  if (fileRecord.expected && !fileRecord.matchesExpected) {
